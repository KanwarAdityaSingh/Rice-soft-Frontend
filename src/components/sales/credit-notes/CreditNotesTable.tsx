  ...
  const filtered = useMemo(() => {
    ...
    return creditNotes.filter((cn) => {
      return (
        ...
        cn.credit_note_date.includes(q) ||
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <FilterDropdown
          ...
          onChange={(v) => setStatusFilter((v ?? '') as CreditNoteStatus | '')}
        />
      </div>
              ...
              <tbody>
                {filtered.map((cn) => (
                  <tr key={cn.id} className="border-b hover:bg-muted/30">
                    ...
                    <td className="p-3">{cn.credit_note_date}</td>
                    <td className="p-3 max-w-[200px] truncate" title={cn.reason ?? ''}>
                      {cn.reason ?? '–'}
                    </td>
                    <td className="p-3">
                      <button
