...
interface SaudaEmailModalProps {
  open: boolean;
...
export function SaudaEmailModal({
  open,
  onOpenChange,
  saudaId,
  onSuccess,
}: SaudaEmailModalProps) {
  ...
  const loadPreview = async () => {
    ...
    try {
      // Fetch sauda data
      const saudaData = await saudasAPI.getSaudaById(saudaId);
      setSauda(saudaData);

      // Fetch notification preview - use saudasAPI method to avoid double /v1
      const preview = await saudasAPI.getNotificationPreview(saudaId);
      if (preview?.email) {
  ...
  const generatePDF = async (): Promise<File | null> => {
    if (!previewRef.current || !sauda) return null;

    try {
      // Create HTML content for PDF (similar to download functionality)
      const printContent = previewRef.current.innerHTML;
      const htmlContent = `
        <!DOCTYPE html>
        <html>
          <head>
            <title>Sauda - ${sauda.id}</title>
            <style>
      ...
      // For now, return HTML blob - backend can convert to PDF
      // In production, you might want to use html2pdf.js or similar
      return new File([blob], `sauda-${saudaId}.html`, { type: 'text/html' });
    } catch (error) {
  ...
  const handleSendEmail = async () => {
    ...
    try {
      ...
      await saudasAPI.sendViaEmail(saudaId, validEmails, {
        customSubject: emailSubject || undefined,
        customHtml: emailHtml || undefined,
        customText: emailText || undefined,
          ...
          <div className="glass rounded-xl sm:rounded-2xl shadow-2xl flex flex-col h-full max-h-[90vh]">
            ...
            <div className="flex items-center justify-between p-6 border-b border-border/50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <Dialog.Title className="text-xl sm:text-2xl font-bold">Send Sauda via Email</Dialog.Title>
