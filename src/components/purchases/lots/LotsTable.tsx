...
export function LotsTable() {
  ...
  const filtered = useMemo(() => {
    return lotsOrdered.filter((lot) => {
      ...
      const displayIso = getLotDisplayDateIso(lot);
      const dateStr = formatLotDate(displayIso).toLowerCase();
      const createdDateStr = formatLotDate(lot.created_at).toLowerCase();
      const matchesSearch =
        ...
        displayIso.toLowerCase().includes(q) ||
              ...
              {filtered.map((lot, index) => (
                <tr key={lot.id} className="border-b border-border/60 hover:bg-muted/30 transition-colors">
                  ...
                  <td className="py-3 px-4 text-sm whitespace-nowrap text-muted-foreground">
                    {formatLotDate(getLotDisplayDateIso(lot))}
                  </td>
