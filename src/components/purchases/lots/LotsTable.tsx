...
export function LotsTable() {
  ...
  // Helper to truncate lot number for display
  const getTruncatedLotNumber = (lotNumber: string): string => {
    if (lotNumber.startsWith('LOT-KAANTA-')) {
      const idPart = lotNumber.replace('LOT-KAANTA-', '');
      return `LOT-KAANTA-${idPart.slice(0, 8)}...`;
    }
    // For non-KAANTA lots, show first 16 characters
    return lotNumber.length > 16 ? `${lotNumber.slice(0, 16)}...` : lotNumber;
  };
                ...
                <tr key={lot.id} className="border-b border-border/60 hover:bg-muted/30 transition-colors">
                  <td className="py-3 px-4">
                    <div className="flex items-center gap-2">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="font-mono text-xs text-muted-foreground">{getTruncatedLotNumber(lot.lot_number)}</span>
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
