...
export function useInwardSlipPasses(sauda_id?: string) {
  ...
  const createInwardSlipPass = async (data: CreateInwardSlipPassRequest) => {
    try {
      const newInwardSlipPass = await inwardSlipPassesAPI.createInwardSlipPass(data);
      await fetchInwardSlipPasses();
      return newInwardSlipPass;
    } catch (err: any) {
      throw err;
    }
  };
  ...
  const updateInwardSlipPass = async (id: string, data: UpdateInwardSlipPassRequest) => {
    try {
      const updatedInwardSlipPass = await inwardSlipPassesAPI.updateInwardSlipPass(id, data);
      await fetchInwardSlipPasses();
      return updatedInwardSlipPass;
    } catch (err: any) {
      throw err;
    }
  };

  const updateInwardSlipPassStatus = async (id: string, status: InwardSlipPassStatus) => {
    try {
      const updatedInwardSlipPass = await inwardSlipPassesAPI.updateInwardSlipPassStatus(id, status);
      await fetchInwardSlipPasses();
      return updatedInwardSlipPass;
    } catch (err: any) {
      throw err;
